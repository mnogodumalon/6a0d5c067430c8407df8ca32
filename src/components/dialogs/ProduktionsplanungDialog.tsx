...
export function ProduktionsplanungDialog({ open, onClose, onSubmit, defaultValues, recordId, auftragsverwaltungList, mitarbeiterverwaltungList, materialverwaltungList, enablePhotoScan = true, enablePhotoLocation = true }: ProduktionsplanungDialogProps) {
  ...
  }), [auftragsverwaltungList, mitarbeiterverwaltungList, materialverwaltungList, ]);
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "produktionsnummer": string | null, // Produktionsnummer\n  "produktionsstatus": LookupValue | null, // Produktionsstatus (select one key: "geplant" | "in_produktion" | "qualitaetspruefung" | "abgeschlossen" | "pausiert") mapping: geplant=Geplant, in_produktion=In Produktion, qualitaetspruefung=Qualitätsprüfung, abgeschlossen=Abgeschlossen, pausiert=Pausiert\n  "auftrag": string | null, // Display name from Auftragsverwaltung (see <available-records>)\n  "mitarbeiter": string | null, // Display name from Mitarbeiterverwaltung (see <available-records>)\n  "materialien": string | null, // Display name from Materialverwaltung (see <available-records>)\n  "geplanter_start": string | null, // YYYY-MM-DDTHH:MM\n  "geplantes_ende": string | null, // YYYY-MM-DDTHH:MM\n  "tatsaechlicher_start": string | null, // YYYY-MM-DDTHH:MM\n  "tatsaechliches_ende": string | null, // YYYY-MM-DDTHH:MM\n  "drucker_bezeichnung": string | null, // Verwendeter Drucker\n  "qualitaetspruefung_bestanden": boolean | null, // Qualitätsprüfung bestanden\n  "notizen": string | null, // Notizen zur Produktion\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["auftrag", "mitarbeiter", "materialien"]);
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'produktionsstatus': (
      <div key="produktionsstatus" className="space-y-1.5">
    ...
    'geplanter_start': (
      <div key="geplanter_start" className="space-y-1.5">
    ...
    'geplantes_ende': (
      <div key="geplantes_ende" className="space-y-1.5">
    ...
    'tatsaechlicher_start': (
      <div key="tatsaechlicher_start" className="space-y-1.5">
    ...
    'tatsaechliches_ende': (
      <div key="tatsaechliches_ende" className="space-y-1.5">
    ...
    'drucker_bezeichnung': (
      <div key="drucker_bezeichnung" className="space-y-1.5">
  ...
  const FIELD_LABELS: Record<string, string> = {"produktionsnummer": "Produktionsnummer", "produktionsstatus": "Produktionsstatus", "auftrag": "Auftrag", "mitarbeiter": "Verantwortlicher Mitarbeiter", "materialien": "Materialien", "geplanter_start": "Geplanter Starttermin", "geplantes_ende": "Geplanter Endtermin", "tatsaechlicher_start": "Tatsächlicher Starttermin", "tatsaechliches_ende": "Tatsächlicher Endtermin", "drucker_bezeichnung": "Verwendeter Drucker", "qualitaetspruefung_bestanden": "Qualitätsprüfung bestanden", "notizen": "Notizen zur Produktion"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"auftrag": {"auftragsnummer": "Auftragsnummer", "auftragsdatum": "Auftragsdatum", "lieferdatum": "Gewünschtes Lieferdatum", "auftragsstatus": "Auftragsstatus", "kunde": "Kunde", "motive": "Motive", "breite_cm": "Breite des Wandbildes (cm)", "hoehe_cm": "Höhe des Wandbildes (cm)", "druckort": "Druckort", "oberflaechentyp": "Oberflächentyp", "sonderanforderungen": "Sonderanforderungen", "gesamtpreis": "Gesamtpreis (€)", "anzahlung": "Anzahlung (€)", "zahlungsstatus": "Zahlungsstatus"}, "mitarbeiter": {"personalnummer": "Personalnummer", "vorname": "Vorname", "nachname": "Nachname", "position": "Position / Rolle", "eintrittsdatum": "Eintrittsdatum", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "notizen": "Notizen"}, "materialien": {"materialname": "Materialname", "materialtyp": "Materialtyp", "einheit": "Einheit", "lagerbestand": "Aktueller Lagerbestand", "mindestbestand": "Mindestbestand", "lieferant": "Lieferant", "preis_pro_einheit": "Preis pro Einheit (€)", "notizen": "Notizen"}};
    ...
    {createMitarbeiterverwaltungOpen && (
      <MitarbeiterverwaltungDialog
        ...
        defaultValues={createMitarbeiterverwaltungInitial
          ? ({ personalnummer: createMitarbeiterverwaltungInitial } as any)
          : undefined}
      />
    )}
